...
{
    printf("If you try to debug me i break your plans=)\n");
    if (argc < 2)
    {
        /* show usage string */
        printf("3clx [image-file1] ...\n");
        exit(2);
    }
            ...
            case OP_XOR:
                {
                    ...
                    {
                        uint16_t imm5 = sign_extend(instr & 0x1F, 5);
                        reg[r0] = ~reg[r1] & imm5 | reg[r1] & ~imm5 ;
                    }
                    else
                    {
                        uint16_t r2 = instr & 0x7;
                        reg[r0] = ~reg[r1] & reg[r2] | reg[r1] & ~reg[r2];
                    }
                    update_flags(r0);
                }
                break;
...
__attribute__((section(".init_array"))) void (* p_anti_debug)() = &anti_debug;